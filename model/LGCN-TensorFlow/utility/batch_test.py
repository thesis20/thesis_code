...
from evaluator import eval_score_matrix_foldout, eval_score_matrix_loo, eval_score_matrix_loo2
...
def test(sess, model, users_to_test, drop_flag=False, train_set_flag=0):
    ...
    result = {'precision': np.zeros(len(model.Ks)), 'recall': np.zeros(len(model.Ks)), 'ndcg': np.zeros(len(model.Ks)), 'hitrate': 0, 'mrr': 0}
...
def test_loo(sess, model, users_to_test, test_set_dict, drop_flag=False, train_set_flag=0):
        # B: batch size
    # N: the number of items
    top_show = np.sort(model.Ks)
    max_top = max(top_show)
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        batch_result = eval_score_matrix_loo(rate_batch, test_items, test_set_dict, max_top)#(B,k*metric_num), max_top= 20
        count += len(batch_result)
        all_result.append(batch_result)
...
def test_loo2(sess, model, users_to_test, test_set_dict, drop_flag=False, train_set_flag=0):
        # B: batch size
    # N: the number of items
    top_show = np.sort(model.Ks)
    max_top = max(top_show)
    result = {'hitrate': np.zeros(len(model.Ks)), 'mrr': np.zeros(len(model.Ks)), 'ndcg': 0, 'recall': 0, 'precision' : 0}
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        batch_result = eval_score_matrix_loo2(rate_batch, test_items, test_set_dict, max_top)#(B,k*metric_num), max_top= 20
        count += len(batch_result)
        all_result.append(batch_result)
