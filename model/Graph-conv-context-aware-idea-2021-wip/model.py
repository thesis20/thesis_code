...
class CSGCN():
    def __init__(self, sess, data):
        ...
        with tf.name_scope('TEST_ACC'):
            ...
            tf.summary.scalar('test_precision_first', self.test_hr_first)
            self.test_recall_first = tf.placeholder(tf.float32)
            tf.summary.scalar('test_recall_first', self.test_ndcg_first)
            self.test_f1_first = tf.placeholder(tf.float32)
            tf.summary.scalar('test_f1_first', self.test_mrr_first)
            self.test_precision_last = tf.placeholder(tf.float32)
            tf.summary.scalar('test_precision_last', self.test_hr_last)
            self.test_recall_last = tf.placeholder(tf.float32)
            tf.summary.scalar('test_recall_last', self.test_ndcg_last)
            self.test_f1_last = tf.placeholder(tf.float32)
            tf.summary.scalar('test_f1_last', self.test_mrr_last)
