...
class LoadDataset():
    ...
    def count_dimensions(self):
        ...
        self.user_offset_to_id_dict = dict(
            (v, k) for k, v in user_offset_dict.items())
        self.item_offset_dict = item_offset_dict
        self.item_offset_to_id_dict = dict(
            (v, k) for k, v in item_offset_dict.items())
    ...
    def build_dictionaries(self):
        ...
        for _, row in self.train_df.iterrows():
            ...
            if row[self.userid_column_name] not in train_set_user_pos_interactions:
                ...
                train_set_user_pos_interactions[row[self.userid_column_name]].append(pos_interaction)
        ...
        for movieId in self.train_df[self.itemid_column_name].unique():
            ...
            for sideinfo_index in sideinfo_indexes:
                try:
                    ...
                    print(f'index: {sideinfo_index}, offset: {item_offset}, movieId: {movieId}')
                    exit()
    ...
    def get_test_context_combinations(self):
        ...
        for _, row in self.test_df.iterrows():
            ...
            for context in self.context_list:
                context_list.append(
                    self.context_offset_dict[context + str(row[context])])
